...
namespace _3._3b.GeneticAlg
{
    class GeneticAlgorithm
    {
        ...
        // нет файлов размером больше дискеты
        /*private bool IsCorrectData(int[] Sizes, int Capacity)
        {
            for(int i =0; i<Sizes.Length; i++)
            {
                if (Sizes[i] > Capacity) return false;
            }
            return true;
        }*/
        ...
        {
            /*if (!IsCorrectData(sizes, capacity))
            {
                return answer;
            } */
        ...
        //представление хромосомы в виде строки для логирования
        public string ToString(List<Chromosome> chs)
        {
            string s = "";
            foreach (Chromosome ch in chs)
            {
                int[] arr = ch.GetCurrentSolve(sizes);
                s += "   { ";
