...
namespace Labyrinth
{
    class DataGridViewUtils
    {
        ...
        public static void ShowDataGridView(DataGridView dgv, int n)
        {
            ...
            dgv.Width = dgv.Columns[0].Width * n + n;//колонок
            dgv.Height = dgv.Rows[0].Height * n + n;//строк
        ...
        public static bool CheckDataGridView(DataGridView dgv)
        {
            ...
            int n_in = 0;//количество входов
            int n_out = 0;//количество выходов

            //проверяем наличие одного входа и одного выхода. должен быть ровно один!
        ...
        public static void RandomDataGridView(DataGridView dgv, Random rnd, ref Point start, ref Point finish)
        {
            ...
            for (int i = 0; (i < n_col); i++)
            {
                for (int j = 0; (j < n_str); j++)
                {
                    ...
                    if (doWall)
                    {
                        dgv[i, j].Style.BackColor = Color.Black;//стена
                    }
                    else
                    {
                        dgv[i, j].Style.BackColor = Color.White;//или проход
                    }
                }
            }
            //делаем вход и выход
            ...
            start = new Point(x, y);
            dgv[x, y].Style.BackColor = Color.Blue;//вход
            do
            {
                x = rnd.Next(1, n_col);
                y = rnd.Next(1, n_str);               
                if (dgv[x, y].Style.BackColor != Color.Blue)//если не вход
                {
                    dgv[x, y].Style.BackColor = Color.DarkBlue;//делам тек клетку выходом
                }
        ...
        public static void ClearWay(DataGridView dgv)
        {
            ...
            for (int i = 0; (i < n_col); i++)
            {
                for (int j = 0; (j < n_str); j++)
                {
                    if (dgv[i, j].Style.BackColor == Color.Green)//зеленые в проход
                    {
                        dgv[i, j].Style.BackColor = Color.White;
                    }
                    else
                        if (dgv[i, j].Style.BackColor == Color.Red)//красные в стены
                        {
